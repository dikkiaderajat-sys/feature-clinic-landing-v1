...
const server=http.createServer(async(req,res)=>{
  try {
    ...
    if(pathname==='/__qa/timeline'||pathname==='/__qa/reduced'){
      let html=await fs.readFile(path.join(root,'clinic/index.html'),'utf8');
      if(pathname.endsWith('reduced'))html=html.replace('href="/clinic/styles.css"','href="/__qa/reduced.css"');
    ...
    if(pathname==='/' || pathname==='/clinic') { res.writeHead(302, {Location:'/clinic/'}); res.end(); return; }
...
server.listen(4173,'127.0.0.1',()=>console.log('Local: http://127.0.0.1:4173/clinic/'));
